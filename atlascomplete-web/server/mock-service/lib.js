...
class MockLib {
  ...
  async loadMock(pathToMock, req) {
    const rawMockData = await this.__loadMock(path.resolve(__dirname, './../../mocks/' + pathToMock));
  ...
  serveMockAsTree(req, res, pathToMock, keyField, parentKeyField) {
    this.loadMock(pathToMock, req).then((data) => {
  ...
  serveMock(req, res, pathToMock) {
    this.loadMock(pathToMock, req).then(
  ...
  serveFailed(req, res, error) {
    this.serveMockData(req, res, 500, void 0, [{
      code: '100100',
      type: 'ERROR',
  ...
  serveMockData(req, res, statusCode, info, messages, meta) {
    const data = info.hasOwnProperty('$payload') ? info.$payload : info;
    const headers = info.hasOwnProperty('$headers') ? info.$headers : {};
